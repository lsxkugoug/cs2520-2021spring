...
{
    int  sk, recv_count, first_pkt_flag,path_count;
  ...
  if( ret != 0 ) {
    ...
    start = E_get_time(); /*sp_time*/
    checkpoint = start;/*sp_time*/
    ...
    if(Send_Flag == 1) {
        ...
        while (1) {
            ...
            if (first_pkt_flag) {
                start = E_get_time();
                /* we calc start time as local clock when first packet arrives */
                /* alternatively we could take the time stamp time on the first packet, but if clock skew, this is bad */
                first_pkt_flag = 0;
                assert(Num_pkts > 0);
                pkt_seq_array = (int *)malloc(sizeof(int)*(Num_pkts+1));
                for(i=1; i<=Num_pkts; i++) {
                    pkt_seq_array[i] = 0;
                }
                last_seq = -1;
            ...
            if (pkt_seq_array[ntohl(f_pkt->seq_num)] != 0) {
                duplicates++;
                continue;
            }
            ...
            if(oneway_time<=65000) {
                recv_count++;
                pkt_seq_array[ntohl(f_pkt->seq_num)]= 1;
            }else {
                continue;
            }
            ...
            if (verbose_mode == 1 && last_seq >= seq_checkpoint) {
                ...
                bytes_checkpoint = 0;
                /*Check the loss*/
                for(i=seq_checkpoint;i>seq_checkpoint-100;i--) {
                    if(pkt_seq_array[i]==0) {
                        checkpoint_loss++;
                    }
                }
                if(checkpoint_loss>=50) {
                    unavail+=1;
                }
                checkpoint_loss=0;
                seq_checkpoint += 100;
                if (seq_checkpoint < last_seq) {
                    unavail+= (last_seq - (last_seq % 100) + 100 -seq_checkpoint)/100;
                    seq_checkpoint = last_seq - (last_seq % 100) + 100;
                }
                if (fileflag == 1) {
        ...
        /*Compute available time and then compute Availability and Unavailability */
        total_time = duration_now / 1000000;
        Availability = (total_time-unavail)/total_time *100;
        Unavailability = unavail/total_time*100;
      ...
      running_std_dev = 0;  /* no std dev on sample of 1 */
        ...
        sprintf(results_str,
                ...
                "- Availability:\t%.6f%% \r\n"
                "- Unavailability:\t%.6f%% \r\n",
                recv_count, Num_pkts,Num_pkts-recv_count, Num_bytes, rate_now,Availability,Unavailability);
    ...
    while( --argc > 0 ) {
        ...
        if( !strncmp( *argv, "-p", 2 ) ){
            ...
            printf( "Usage: sp_bflooder\n"
                    ...
                    "\t[-b <size>       ] : 1000 size of the packets (in bytes)\n"
